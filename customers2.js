...
// image
  ...
  // address
    ...
    let stNameAddress = customers2.location.street.name
    let cityAddress = customers2.location.city
    let stateName = customers2.location.state
    nameToAbbr(stateName)
    console.log(nameToAbbr(stateName))
    let postal = customers2.location.postcode
    let address = (stNumAddress + ' ' + stNameAddress + ' ' + cityAddress + ', ' + nameToAbbr(stateName) + ' ' + postal)
    // DOB
    let bday = moment(customers2.dob.date).format("MMM Do YY")
    let dob = "DOB: " + bday
    // register
    let registered = moment(customers2.registered.date).format("MMM Do YY")
    let register = "Customer since: " + registered
  personDiv.innerHTML += `<div> <img src=${srcURL} <br>
